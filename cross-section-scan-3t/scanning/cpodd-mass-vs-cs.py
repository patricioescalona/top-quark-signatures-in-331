...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Scan a mass parameter in the local pp -> tt t~ MadGraph process and "
            "save the summary table under cross-section-scan-3t/results/."
        )
    )
    parser.add_argument(
        "--mg5-bin",
    ...
    parser.add_argument(
        "--output-name",
        default="bsm-tttbar-scan",
        help="Base name for the summary file written in cross-section-scan-3t/results/.",
    )
    parser.add_argument(
        "--output-format",
...
set run_card ebeam1 {ebeam1}
set run_card ebeam2 {ebeam2}
set nevents {nevents}
done
"""
...
def run_mg5(
    ...
    with tempfile.TemporaryDirectory(prefix="mg5-scan-") as tmpdir:
        ...
        card_path.write_text(
            build_card(process, pdg, mass, tanphi, ebeam1, ebeam2, nevents),
            encoding="utf-8",
        )
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        print(f"ebeam1       : {args.ebeam1}")
        print(f"ebeam2       : {args.ebeam2}")
    ...
    for mass in masses:
        ...
        if cross_section is not None and cross_section_error is not None:
            ...
            print(f"m={mass_text} -> xs unavailable [{status}]", flush=True)
