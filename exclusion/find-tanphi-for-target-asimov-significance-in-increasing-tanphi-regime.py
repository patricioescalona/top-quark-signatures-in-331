...
MINIMUM_TANPHI_IN_INCREASING_REGIME = 1.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        required=True,
        help="Starting tanphi value used in the increasing-tanphi regime search.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--maximum-tanphi-in-increasing-regime",
        type=float,
        default=1.0e2,
        help="Largest tanphi value allowed while expanding the upper bracket. Default: 1e2",
    )
    parser.add_argument(
    ...
    if args.tanphi_seed <= MINIMUM_TANPHI_IN_INCREASING_REGIME:
        parser.error("--tanphi-seed must satisfy tanphi > 1 in the increasing regime.")
    ...
    if args.maximum_tanphi_in_increasing_regime <= MINIMUM_TANPHI_IN_INCREASING_REGIME:
        parser.error("--maximum-tanphi-in-increasing-regime must be greater than 1.")
...
class IncreasingTanphiGeneratedSignalPointEvaluator:
...
def find_tanphi_bracketing_interval_for_target_asimov_significance_in_increasing_regime(
    evaluator: IncreasingTanphiGeneratedSignalPointEvaluator,
    seed_evaluation: GeneratedSignalPointEvaluation,
    ...
    if seed_evaluation.final_asimov_significance < target_asimov_significance:
        ...
        for iteration_index in range(1, args.maximum_number_of_bracketing_iterations + 1):
            candidate_tanphi_value = min(
                ...
                * args.tanphi_growth_factor_during_bracketing,
                args.maximum_tanphi_in_increasing_regime,
            )
        ...
        raise ValueError(
            "Could not bracket the target Asimov significance from above before reaching "
            f"tanphi={args.maximum_tanphi_in_increasing_regime:.12g}."
        )
    ...
    for iteration_index in range(1, args.maximum_number_of_bracketing_iterations + 1):
        ...
        if candidate_tanphi_value <= MINIMUM_TANPHI_IN_INCREASING_REGIME:
            lower_bracketing_evaluation = evaluator.evaluate_generated_signal_point_at_tanphi(
                MINIMUM_TANPHI_IN_INCREASING_REGIME
            )
            if significance_is_within_target_tolerance(
            ...
            if lower_bracketing_evaluation.final_asimov_significance > target_asimov_significance:
                raise ValueError(
                    "Even tanphi=1 stays above the target Asimov significance, so no "
                    "solution was found in the tanphi >= 1 increasing regime."
                )
            return lower_bracketing_evaluation, upper_bracketing_evaluation, iteration_index
    ...
    raise ValueError(
        "Could not bracket the target Asimov significance from below before exhausting "
        "the allowed number of bracketing iterations."
    )


def refine_tanphi_with_logarithmic_bisection_for_target_asimov_significance_in_increasing_regime(
    evaluator: IncreasingTanphiGeneratedSignalPointEvaluator,
    lower_bracketing_evaluation: GeneratedSignalPointEvaluation,
    upper_bracketing_evaluation: GeneratedSignalPointEvaluation,
    args: argparse.Namespace,
...
def write_search_summary_text(
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "Increasing-tanphi Asimov significance search",
...
def main() -> int:
    ...
    evaluator = IncreasingTanphiGeneratedSignalPointEvaluator(args)
    ...
    ) = find_tanphi_bracketing_interval_for_target_asimov_significance_in_increasing_regime(
        evaluator,
        seed_evaluation,
        args,
    )
    best_evaluation, number_of_bisection_iterations, stopping_reason = (
        refine_tanphi_with_logarithmic_bisection_for_target_asimov_significance_in_increasing_regime(
            evaluator,
            lower_bracketing_evaluation,
            upper_bracketing_evaluation,
            args,
